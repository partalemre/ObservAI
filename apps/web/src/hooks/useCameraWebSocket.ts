...
export interface CameraLiveData {
  current: number // Current people inside
  entriesHour: number // Entries in the last hour
  exitsHour: number // Exits in the last hour
...
const generateMockData = (): CameraLiveData => {
  const now = new Date()
...
export const useCameraWebSocket = () => {
  const [mockData, setMockData] = useState<CameraLiveData | null>(null)
  ...
  const {
    ...
    reconnect,
    disconnect,
  } = useRealtimeData<CameraLiveData>('camera', {
    reconnectAttempts: 5,
    reconnectDelay: 1000,
